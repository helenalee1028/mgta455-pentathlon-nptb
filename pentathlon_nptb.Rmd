...
Also, we used single layer neural network to capture the interaction effects. In this step, we looped over node size from 1 to 10 and decay range from 0.1 to 1. Then we used the optimal model to run predictions on test dataset. 
...
The best size is 7 and best decay is 0.8. We then predict based on this model.
...
In keras deep learning models, we tried 2 and 3 hidden layers and compared the prediction performance on test dataset and below are our results. The accuracy scores are both around 0.810 and the AUC scores around 0.886. 
...
In this part, we tuned GBM models 3 times and saved the optimal model from each round of tuning. In the end we choose the one with highest ROC and predict on test dataset. 
...
First, we generate rough parameters to test using manual grid search. The tuning result is as following.
...
As we can see from the table, almost all models perform better when nrounds is 30 compared to 20,so we conclude that the best nrounds may be larger than 30 and further tuning is needed. The best max_depth is 7 and best eta is 0.2.
...
We found the best nrounds is 30. Up to now, we have got all best parameters for xgboost model we need. And we train the final model.
...
The final auc of the best xgboost model is `r round(AUC, 4)` and then we run predictions on test dataset. 
...
```{r}
# regress order size
pentathlon_nptb_os <- pentathlon_nptb_total %>% 
  mutate(total_os_ln = log(total_os))
...
lm_result <- regress(
  ...
  rvar = "total_os_ln", 
  evar = c(
    "message", "age", "income", "education", "children", "freq_endurance", 
    "freq_strength", "freq_water", "freq_team", "freq_backcountry", 
    "freq_winter", "freq_racquet"
  ), 
  data_filter = "buyer == 'yes'"
)
summary(lm_result, sum_check = "rmse")

# use model to predict order size for each customer if they purchase

lm_os_random <- predict(lm_result, pred_data = pentathlon_nptb_rep)$Prediction
lm_os_end <- predict(lm_result, pred_data = pentathlon_nptb_rep, pred_cmd = "message = 'endurance'")$Prediction
lm_os_rac <- predict(lm_result, pred_data = pentathlon_nptb_rep, pred_cmd = "message = 'racquet'")$Prediction
lm_os_water <- predict(lm_result, pred_data = pentathlon_nptb_rep, pred_cmd = "message = 'water'")$Prediction
lm_os_winter <- predict(lm_result, pred_data = pentathlon_nptb_rep, pred_cmd = "message = 'winter'")$Prediction
lm_os_bc <- predict(lm_result, pred_data = pentathlon_nptb_rep, pred_cmd = "message = 'backcountry'")$Prediction
lm_os_strg <- predict(lm_result, pred_data = pentathlon_nptb_rep, pred_cmd = "message = 'strength'")$Prediction
lm_os_team <- predict(lm_result, pred_data = pentathlon_nptb_rep, pred_cmd = "message = 'team'")$Prediction
