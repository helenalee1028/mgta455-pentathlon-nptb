...
First, we generate rough parameters to test.
...
As we can see from the table, when nrounds increase from 10 t0 30, auc increases, so we need further tuning to get the best nrounds. The best max_depth is 6 and best eta is 0.2
...
In this tuning, we specifically tune nrounds. We found the best nrounds is 30. 

Up to now, we have got all best parameters we need. And we train the final model.
```{r eval = FALSE}
params <- list(
  objective = "binary:logistic",
  eta = 0.2,
...
The final auc of the best xgboost model is 0.8868 and the prediction is stored in column：pred_xgb in test set.
